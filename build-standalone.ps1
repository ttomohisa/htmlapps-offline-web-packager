...
$AppCorePath = Join-Path $Root "src\offline-web-packager-core.js"
...
if (-not (Test-Path -LiteralPath $AppCorePath -PathType Leaf)) { throw "Application core script was not found: $AppCorePath" }
$appCoreJs = [System.IO.File]::ReadAllText($AppCorePath, [System.Text.Encoding]::UTF8)
if ($appCoreJs -match "(?i)</script") { throw "Application core script must not contain a literal </script sequence." }
